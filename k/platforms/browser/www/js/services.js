	...
	if (defaultConfig == null)
		defaultConfig = {
			...
			feedRate	: {
				...
				defaultValue: '300',
				type		: 'number',
				placeholder : 'FEED RATE SETS STEPPERS SPEED'
			},
	...
	function tryDraw(constConvert, maxPoint, processedLine) {
		...
		while (i < vec_array.length) {
			...
			if (fromPoint.distance(toPoint) > 5) {
				...
				var color = (depthList[i].x == 1) ? 'rgb(11,' + map(depthList[i].y, 0, 1000, 0, 255) + ', 132)' : 'rgb(200, 50, 20)';
				color = (i <= processedLine) ? 'blue' : color;				
				canvas.add(drawLine(coords, color));
				fromPoint = toPoint;
			}
			i++;
		}
